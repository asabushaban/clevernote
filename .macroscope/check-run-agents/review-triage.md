...
2. **PR review comments** (inline, code-anchored): `GET repos/{owner}/{repo}/pulls/{pull_number}/comments`. Capture each comment's `id`, `user.login`, `user.type`, `author_association`, `path`, `line`, and `body`. (`author_association` is on every REST comment object and feeds the Phase 2 authorization gate — capture it or that gate has nothing to check.) Your GitHub tool is **REST-only and repo-scoped** (no GraphQL), and REST does not expose a review thread's resolved/unresolved flag, so you **cannot** read "is this thread resolved?" directly. Instead judge whether a comment is already handled from the **current code** (does the file already satisfy it?) and from **prior-run activity** in `<prior_runs>` (did an earlier run act on it?). See Phase 5.
...
For each one you can confidently resolve, fix it directly with `write_code` (read the relevant code first with `browse_code`) — a failing lint/test/type check you can correct, a concrete issue a bot pointed at, and so on. The executor commits your edits to the PR branch.
